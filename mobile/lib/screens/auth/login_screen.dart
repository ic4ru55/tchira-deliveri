...
class _LoginScreenState extends State<LoginScreen> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    _slideAnim = Tween<Offset>(begin: const Offset(0, 0.3), end: Offset.zero).animate(CurvedAnimation(parent: _animCtrl, curve: Curves.easeOutCubic));
  ...
  Future<void> _login() async {
    ...
    final succes = await auth.login(email: _emailCtrl.text.trim(), motDePasse: _mdpCtrl.text.trim());
    if (!mounted) return;
    if (succes) {
      Widget ecran;
      ...
      body: Column(children: [
        ...
        Expanded(flex: 3, child: SlideTransition(position: _slideAnim, child: FadeTransition(opacity: _fadeAnim, child:
          Container(
            ...
            child: SingleChildScrollView(padding: const EdgeInsets.fromLTRB(24, 32, 24, 24), child: Form(key: _formKey, child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              TextFormField(
                ...
                decoration: _inputDeco('Mot de passe', Icons.lock_outlined).copyWith(
                  suffixIcon: IconButton(icon: Icon(_mdpVisible ? Icons.visibility_off : Icons.visibility, color: Colors.grey, size: 20), onPressed: () => setState(() => _mdpVisible = !_mdpVisible))),
              ...
              SizedBox(width: double.infinity, height: 52, child: ElevatedButton(
                ...
                style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0D7377), foregroundColor: Colors.white,
                    ...
                    elevation: 3, shadowColor: const Color(0xFF0D7377).withValues(alpha: 0.4)),
              ...
              // Séparateur
              Row(children: [const Expanded(child: Divider()), Padding(padding: const EdgeInsets.symmetric(horizontal: 12), child: Text('Rôles', style: TextStyle(color: Colors.grey.shade400, fontSize: 11))), const Expanded(child: Divider())]),
