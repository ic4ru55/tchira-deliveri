...
class _RegisterScreenState extends State<RegisterScreen>
  ...
  void _allerEtapeSuivante() {
    ...
    if (_etape == 1) {
      if (_nomCtrl.text.isEmpty || _emailCtrl.text.isEmpty || _telCtrl.text.isEmpty || !_emailCtrl.text.contains('@')) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Remplis tous les champs correctement'), backgroundColor: Colors.red));
        return;
      }
  ...
  void _retourEtape() {
    _animCtrl.forward(from: 0); setState(() => _etape = _etape > 0 ? _etape - 1 : 0);
  }

  Future<void> _register() async {
    if (!_formKey.currentState!.validate()) return;
    final auth = context.read<AuthProvider>();
    final succes = await auth.register(nom: _nomCtrl.text.trim(), email: _emailCtrl.text.trim(), motDePasse: _mdpCtrl.text.trim(), telephone: _telCtrl.text.trim(), role: _role);
    if (!mounted) return;
    if (succes) {
      Navigator.pushReplacement(context, MaterialPageRoute(builder: (_) => auth.estClient ? const HomeClient() : const HomeLibreur()));
    }
  }
          ...
          child: Column(children: [
            Row(children: [
              if (_etape > 0) GestureDetector(
                onTap: _retourEtape,
                child: Container(padding: const EdgeInsets.all(8), decoration: BoxDecoration(color: Colors.white.withValues(alpha: 0.15), borderRadius: BorderRadius.circular(10)),
            ...
            // Logo
            Container(width: 70, height: 70, decoration: BoxDecoration(shape: BoxShape.circle, border: Border.all(color: Colors.white, width: 2.5), color: Colors.white.withValues(alpha: 0.1)),
            ...
            Row(mainAxisAlignment: MainAxisAlignment.center, children: List.generate(3, (i) => AnimatedContainer(
              ...
              decoration: BoxDecoration(color: _etape == i ? Colors.white : Colors.white.withValues(alpha: 0.3), borderRadius: BorderRadius.circular(4)),
            ))),
          ]),
        ),
          ...
          child: FadeTransition(opacity: _fadeAnim, child: SingleChildScrollView(
            ...
            child: Form(key: _formKey, child: Column(children: [
              ...
              if (_etape == 0) ...[
                ...
                const Text('Choisis ton type de compte pour commencer', style: TextStyle(color: Colors.grey, fontSize: 13), textAlign: TextAlign.center),
              ...
              if (_etape == 1) ...[
                ...
                const SizedBox(height: 8),
                const Text('Ces informations seront visibles dans ton profil', style: TextStyle(color: Colors.grey, fontSize: 13), textAlign: TextAlign.center),
                const SizedBox(height: 28),
                _champ(_nomCtrl, 'Nom complet', Icons.person_outlined, validateur: (v) => v!.isEmpty ? 'Nom requis' : null),
                const SizedBox(height: 14),
                _champ(_emailCtrl, 'Email', Icons.email_outlined, clavier: TextInputType.emailAddress, validateur: (v) { if (v!.isEmpty) return 'Email requis'; if (!v.contains('@')) return 'Email invalide'; return null; }),
                const SizedBox(height: 14),
                _champ(_telCtrl, 'Téléphone', Icons.phone_outlined, clavier: TextInputType.phone, validateur: (v) => v!.isEmpty ? 'Téléphone requis' : null),
                const SizedBox(height: 28),
                _boutonSuivant('Continuer →', () => _allerEtapeSuivante()),
              ],
              ...
              if (_etape == 2) ...[
                ...
                Container(padding: const EdgeInsets.all(14), margin: const EdgeInsets.only(bottom: 20),
                  decoration: BoxDecoration(color: const Color(0xFF0D7377).withValues(alpha: 0.06), borderRadius: BorderRadius.circular(12), border: Border.all(color: const Color(0xFF0D7377).withValues(alpha: 0.2))),
                  child: Column(children: [
                    _ligneRecap(Icons.person, _nomCtrl.text),
                    const SizedBox(height: 8),
                    _ligneRecap(Icons.email_outlined, _emailCtrl.text),
                    const SizedBox(height: 8),
                    _ligneRecap(Icons.phone_outlined, _telCtrl.text),
                  ])),

                TextFormField(
                  controller: _mdpCtrl, obscureText: !_mdpVisible,
                  decoration: _inputDeco('Mot de passe', Icons.lock_outlined).copyWith(
                    suffixIcon: IconButton(icon: Icon(_mdpVisible ? Icons.visibility_off : Icons.visibility, color: Colors.grey), onPressed: () => setState(() => _mdpVisible = !_mdpVisible)),
                  ),
                ...
                if (auth.erreur != null) ...[
                  ...
                  Container(width: double.infinity, padding: const EdgeInsets.all(12),
                    ...
                    child: Row(children: [const Icon(Icons.error_outline, color: Color(0xFFDC2626), size: 18), const SizedBox(width: 8), Expanded(child: Text(auth.erreur!, style: const TextStyle(color: Color(0xFFDC2626), fontSize: 13)))])),
                ],
                const SizedBox(height: 28),
                SizedBox(width: double.infinity, height: 52, child: ElevatedButton.icon(
                  ...
                  style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0D7377), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
                )),
              ],
    ...
    return Expanded(child: GestureDetector(
      ...
      child: AnimatedContainer(duration: const Duration(milliseconds: 200),
        ...
        decoration: BoxDecoration(
          color: sel ? couleur : Colors.white,
          borderRadius: BorderRadius.circular(20),
          border: Border.all(color: sel ? couleur : const Color(0xFFE2E8F0), width: sel ? 2 : 1),
          boxShadow: sel ? [BoxShadow(color: couleur.withValues(alpha: 0.3), blurRadius: 16, offset: const Offset(0, 6))] : [BoxShadow(color: Colors.black.withValues(alpha: 0.04), blurRadius: 8, offset: const Offset(0, 2))],
        ),
        child: Column(children: [
          ...
          AnimatedContainer(duration: const Duration(milliseconds: 200), width: 24, height: 24,
            decoration: BoxDecoration(shape: BoxShape.circle, color: sel ? Colors.white : Colors.transparent, border: Border.all(color: sel ? Colors.white : const Color(0xFFE2E8F0), width: 2)),
  ...
  Widget _boutonSuivant(String label, VoidCallback onTap) => SizedBox(width: double.infinity, height: 52,
    child: ElevatedButton(onPressed: onTap, style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFF0D7377), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(14))),
  ...
  Widget _champ(TextEditingController ctrl, String label, IconData icone, {TextInputType clavier = TextInputType.text, String? Function(String?)? validateur}) =>
    TextFormField(controller: ctrl, keyboardType: clavier, validator: validateur, decoration: _inputDeco(label, icone));

  Widget _ligneRecap(IconData ic, String val) => Row(children: [Icon(ic, size: 14, color: const Color(0xFF0D7377)), const SizedBox(width: 8), Expanded(child: Text(val, style: const TextStyle(fontSize: 13, color: Color(0xFF0D7377), fontWeight: FontWeight.w500)))]);
